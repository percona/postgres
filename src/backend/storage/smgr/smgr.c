...
smgrexists(SMgrRelation reln, ForkNumber forknum)
{
	...
	HOLD_INTERRUPTS();
	ret = smgr_exists_next(reln, forknum, 0);
	RESUME_INTERRUPTS();
			 ...
			 int nblocks)
{
	...
	HOLD_INTERRUPTS();
	ret = smgr_prefetch_next(reln, forknum, blocknum, nblocks, 0);
	RESUME_INTERRUPTS();
			   ...
			   BlockNumber blocknum)
{
	...
	HOLD_INTERRUPTS();
	ret = smgr_maxcombine_next(reln, forknum, blocknum, 0);
	RESUME_INTERRUPTS();
 ...
 */
void
smgr_startreadv_next(PgAioHandle *ioh, SMgrRelation reln, ForkNumber forknum,
					 BlockNumber blocknum, void **buffers, BlockNumber nblocks,
					 SmgrChainIndex chain_index)
{
	SMGR_CHAIN_LOOKUP(smgr_startreadv);

	smgrsw[reln->smgr_chain.chain[chain_index]].smgr_startreadv(ioh, reln, forknum, blocknum,
																buffers, nblocks, chain_index);
}
...
{
	HOLD_INTERRUPTS();
	smgr_startreadv_next(ioh, reln, forknum, blocknum, buffers, nblocks, 0);
	RESUME_INTERRUPTS();
}
...
int
smgr_fd_next(SMgrRelation reln, ForkNumber forknum, BlockNumber blocknum,
			 uint32 *off, SmgrChainIndex chain_index)
{
	SMGR_CHAIN_LOOKUP(smgr_fd);

	return smgrsw[reln->smgr_chain.chain[chain_index]].smgr_fd(reln, forknum, blocknum, off, chain_index);
}
...
smgrfd(SMgrRelation reln, ForkNumber forknum, BlockNumber blocknum, uint32 *off)
{
	...
	fd = smgr_fd_next(reln, forknum, blocknum, off, 0);
