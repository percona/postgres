...
command_like([ 'pg_waldump', '-k', $node->data_dir. '/global', $node->data_dir . '/pg_wal/' . $start_walfile ],
...
command_fails_like(
	[ 'pg_waldump', '-k', $node->data_dir. '/global', '-p', $node->data_dir, '--start', $start_lsn ],
...
command_fails_like(
	[ 'pg_waldump', '--quiet', '-k', $node->data_dir. '/global', '-p', $node->data_dir, '--start', $start_lsn ],
	qr/error: error in WAL record at/,
...
# wasn't a pointer to the start of a record.
{
	...
	@cmd = (
		'pg_waldump', '-k', $node->data_dir. '/global',
		'--start', $new_start,
		$node->data_dir . '/pg_wal/' . $start_walfile);
...
sub test_pg_waldump
{
	...
	@cmd = (
		'pg_waldump', '-k', $node->data_dir. '/global', '-p', $node->data_dir,
