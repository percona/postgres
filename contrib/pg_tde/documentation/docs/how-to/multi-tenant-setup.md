...
2. Add a principal key
    ...
    * `key_action` defines if a principal key is to be generated or already exist in the key provider. The default value `generate_if_not_exists` means that a new key will be generated if no key was found in the rpovider. The `generate` value requires a new key to be generated by `pg_tde` while the value `existing` will use a key already stored in the key provider.
