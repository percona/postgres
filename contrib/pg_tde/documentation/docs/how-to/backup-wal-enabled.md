...
!!! note
    - The `-E` flag only works with the `-X stream` option (default). It is not compatible with `-X none` or `-X fetch`. For more information, see [Backup method compatibility with WAL encryption](#backup-method-compatibility-with-wal-encryption).
