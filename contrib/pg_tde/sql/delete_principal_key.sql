...
SELECT pg_tde_set_key_using_global_key_provider('test-db-key', 'file-provider', 'generate_if_not_exists');
...
SELECT pg_tde_set_default_key_using_global_key_provider('defalut-key', 'file-provider', 'generate_if_not_exists');
SELECT pg_tde_set_key_using_global_key_provider('test-db-key', 'file-provider', 'generate_if_not_exists');
CREATE TABLE test_table (id int, data text) USING tde_heap;
SELECT pg_tde_delete_key();
SELECT provider_id, provider_name, key_name FROM pg_tde_key_info();
